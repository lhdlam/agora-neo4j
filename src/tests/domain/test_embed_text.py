...
class TestBuildEmbedText:
    def test_title_always_present(self) -> None:
        text = build_embed_text(_make_sell_listing())
        assert "iPhone 14 Pro 256GB" in text

    def test_description_included_when_present(self) -> None:
        text = build_embed_text(_make_sell_listing(description="Like new, boxed"))
        assert "Like new, boxed" in text

    def test_description_absent_when_none(self) -> None:
        text = build_embed_text(_make_sell_listing(description=None))
    ...
    def test_tags_joined_with_space(self) -> None:
        text = build_embed_text(_make_sell_listing(tags=["apple", "smartphone"]))
    ...
    def test_buyer_info_fields_included(self) -> None:
        text = build_embed_text(_make_buy_listing())
        assert "Apple" in text
        assert "like-new" in text

    def test_no_seller_info_no_crash(self) -> None:
        text = build_embed_text(_make_sell_listing(seller_info=None))
        assert isinstance(text, str)

    def test_returns_single_line_string(self) -> None:
        text = build_embed_text(_make_sell_listing(tags=["a", "b"], description="desc"))
        assert isinstance(text, str)
        assert "\n" not in text
